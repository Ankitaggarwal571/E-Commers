...
import { addBrandsAction } from '../Store/ActionCreateors/BrandsAction'
import LeftNav from './LeftNav'


export default function BrandsAdd() {
    const [name,setName]=useState()
    const BrandDataState=useSelector((state)=>state.BrandsState)
// console.log(name)
    ...
    function postData(e){
        e.preventDefault()
        var item=BrandDataState.find((item)=>item.name===name)
        if(item){
            alert("Already Exits")
        }
        else{
           dispatch(addBrandsAction({name:name}))
